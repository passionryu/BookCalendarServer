...
public class MypageController {
    ...
    /**
     * 독서 수 & 독후감 작성 수 조회 API
     *
     * @param customUserDetails 인증된 유저의 정보 객체
     * @return 독서 수 & 독후감 작성 수 DTO 반환
     */
    @Operation(summary = "독서 수 & 독후감 작성 수 조회 API", description = "독서 수와 독후감 작성 수를 조회하는 기능이다.  ",
            responses = {
                    @ApiResponse(responseCode = "200", description = "독서 수와 독후감 작성수가 정상적으로 되었습니다."),
                    @ApiResponse(responseCode = "401", description = "유저 인증 오류"),
                    @ApiResponse(responseCode = "500", description = "서버 내부 오류")
            })
